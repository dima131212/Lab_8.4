...
public class IdGeneration {
    ...
    public long getCurrentIdFromSequence() throws SQLException {
    	String sql = "SELECT MAX(id) FROM movie";  // Запрос для получения максимального id из таблицы movie
        try (PreparedStatement statement = connection.prepareStatement(sql)) {
            ResultSet resultSet = statement.executeQuery();
            if (resultSet.next()) {
                return resultSet.getLong(1);  // Возвращаем максимальный id
            } else {
                throw new SQLException("Не удалось получить последний id фильма из таблицы movie");
            }
        }
    }
